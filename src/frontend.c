...
char* as_f_compound(AST* ast, List* list) {
...
char* as_f_function(AST* ast, List* list)
{
  char* name = ast->name;
  ...
  {
    AST* farg = (AST*) as_val->children->items[i];
...
char* as_f_assignment(AST* ast, List* list)
{
...
char* as_f_variable(AST* ast, List* list) {
  char* s = calloc(1, sizeof(char));
...
char* as_f_call(AST* ast, List* list)
{
  char* s = calloc(1, sizeof(char));
  ...
  if (list->size)
  {
    if (((AST*)list->items[0])->type == AST_ASSIGNMENT)
      addl_size = 0;
  }
...
char* as_f_statement_return(AST* ast, List* list)
{
  char* s = calloc(1, sizeof(char));
...
char* as_f_int(AST* ast, List* list)
{
...
char* as_f_string(AST* ast, List* list)
{
  List* chunks = str_to_hex_chunks(ast->string_value);
...
char* as_f_root(AST* ast, List* list)
{
...
char* as_f_access(AST* ast, List* list)
{
...
char* as_f_binop(AST* ast, List* list)
{
  char* s = calloc(1, sizeof(char));
